...
def flatten_weights(policy):
    """Flattens the GAPolicy weights and biases into a 1D numpy array."""
...
def unflatten_weights(policy, flat_weights):
    """Reshapes a 1D array back into the GAPolicy weight matrices."""
...
def train_cmaes(generations=300, population_size=100):
    print(f"Starting CMA-ES Training: Pop={population_size}, Gens={generations}")

    # Initialize a dummy policy just to get the shapes and initial random weights
    dummy_policy = GAPolicy()
    initial_weights = flatten_weights(dummy_policy)

    # Initialize CMA-ES optimizer
    # We use a relatively large initial sigma to encourage exploration
    ...
    for generation in range(generations):
        ...
        # 1. Ask: The optimizer gives us a population of weight vectors to test
        for _ in range(optimizer.population_size):
            ...
            # Evaluate fitness (using multiple episodes to smooth out RNG)
            # CMA-ES normally minimizes, so we flip the sign of the fitness score later
            fitness = evaluate_policy(test_policy, num_episodes=5)

            # The cmaes library minimizes, so we return negative fitness as the cost
        ...
        # 2. Tell: We pass the evaluated solutions back to update the covariance matrix
        ...
        # Stop early if the distribution collapses
