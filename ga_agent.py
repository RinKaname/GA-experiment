...
class GAPolicy:
    def __init__(self, state_size=9, hidden_size=16, action_size=3):
        ...
        self.W1 = np.random.randn(hidden_size, state_size) * np.sqrt(2.0 / state_size)
...
class GARNNPolicy:
    def __init__(self, state_size=9, hidden_size=16, action_size=3):
        ...
        self.W_ih = np.random.randn(hidden_size, state_size) * np.sqrt(2.0 / state_size)
        self.W_hh = np.random.randn(hidden_size, hidden_size) * np.sqrt(2.0 / hidden_size)
        self.b_h = np.zeros(hidden_size)

        self.W_ho = np.random.randn(action_size, hidden_size) * np.sqrt(2.0 / hidden_size)
        self.b_o = np.zeros(action_size)
