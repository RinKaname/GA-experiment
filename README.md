...
3. **CMA-ES:** A powerful gradient-free optimizer that successfully managed resources but struggled with the 3-way faction balancing.
4. **Deep Reinforcement Learning (PPO):** Mastered short-term resource management (zero starvation) but failed long-horizon planning, depleting resources and dying in Year 6.
5. **Model-Based RL (Micro-Dreamer):** A PyTorch implementation demonstrating fully differentiable training via imagined futures inside a learned World Model.
6. **Agentic SLMs (Gemma 4):** Demonstrated superior algebraic reasoning but struggled with multi-turn political foresight. Evaluated via a custom text wrapper and Metamemory loops.
...
### 3. Model-Based RL (Dreamer)
Requires `torch`.
```bash
python train_dreamer.py     # Trains World Model and Actor
python benchmark_dreamer.py # Evaluates the Dreamer Actor
```
